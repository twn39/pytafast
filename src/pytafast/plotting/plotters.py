...
class BasePlotter:
    ...
    def _map_inputs(self, chart):
        ...
        if (
            "inOpen" in params
            and "inHigh" in params
            and "inLow" in params
            and "inClose" in params
        ):
...
class LinePlotter(BasePlotter):
    ...
    def plot(self, chart):
        ...
        name = self.kwargs.get(
            "name", f"{self.indicator_fn.__name__}({self.args[0] if self.args else ''})"
        )
...
class BandPlotter(BasePlotter):
    ...
    def plot(self, chart):
        ...
        upper, middle, lower = self.indicator_fn(
            *inputs, *self.args, **indicator_kwargs
        )
...
class OscillatorPlotter(BasePlotter):
    ...
    def plot(self, chart):
        ...
        trace = go.Scatter(
            x=chart.dt, y=y, mode="lines", name=name, line=dict(color=color)
        )
        traces = [trace]
        for hl in hlines:
            traces.append(
    ...
    def plot(self, chart):
        ...
        traces = [
            go.Bar(
                ...
                y=hist,
                name="MACD Hist",
                marker_color=np.where(hist >= 0, inc, dec),
            ),
...
class AroonPlotter(BasePlotter):
    ...
    def plot(self, chart):
        ...
        traces = [
            go.Scatter(
                x=chart.dt, y=aroonup, name="Aroon Up", line=dict(color="green")
            ),
            go.Scatter(
                x=chart.dt, y=aroondown, name="Aroon Down", line=dict(color="red")
            ),
        ]
...
class ADXPlotter(BasePlotter):
    ...
    def plot(self, chart):
        ...
        traces = [
            ...
            go.Scatter(
                x=chart.dt, y=mdi, name=f"-DI({n})", line=dict(color="red", dash="dot")
            ),
        ]
...
class CLVPlotter(BasePlotter):
    ...
    def plot(self, chart):
        ...
        v = np.where(
            denom != 0, ((chart.C - chart.L) - (chart.H - chart.C)) / denom, 0.0
        )
            ...
            if len(hit_idx) > 0:
                chart.main_traces.append(
                    go.Scatter(
                        ...
                        hovertext=[
                            f"{p_name[3:]} ({'Bullish' if res[idx] > 0 else 'Bearish'})"
                            for idx in hit_idx
                        ],
                        hoverinfo="text",
...
class EnvelopePlotter(BasePlotter):
    ...
    def plot(self, chart):
        ...
        chart._add_overlay(lower, "Env Lower", "rgba(0, 0, 255, 0.3)", 1, "dot")
        chart.main_traces[-1].fill = "tonexty"
