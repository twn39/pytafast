...
def test_smi_alignment(data_file):
    ...
    r_res = run_r_ttr(
        data_path,
        "res <- SMI(cbind(high, low, close), n=13, nFast=2, nSlow=25, nSig=9)",
    )
...
def test_roc_alignment(data_file):
    ...
    r_res = run_r_ttr(
        data_path, 'res <- data.frame(roc=ROC(close, n=10, type="discrete") * 100)'
    )
...
def test_chaikin_vol_alignment(data_file):
    ...
    r_res = run_r_ttr(
        data_path,
        "res <- data.frame(chv=chaikinVolatility(cbind(high, low), n=10) * 100)",
    )
