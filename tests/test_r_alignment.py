...
def assert_aligned(p_val, r_val, indicator_name="Unknown", rtol=1e-5, atol=1e-5):
    ...
    np.testing.assert_allclose(
        ...
        rtol=rtol,
        atol=atol,
        err_msg=f"Mismatch in {indicator_name}",
    )
...
@pytest.mark.xfail(
    reason="Standard Deviation bias difference (Population vs Sample N-1 in R)"
)
@pytest.mark.parametrize("reference_data", DATA_FILES, indirect=True)
...
# --- 6. Known Mismatches ---
@pytest.mark.xfail(
    reason="SMI in pytafast follows Blau/TA-Lib standard which differs from TTR zero padding seeds"
)
@pytest.mark.parametrize("reference_data", DATA_FILES, indirect=True)
...
@pytest.mark.xfail(
    reason="CMO in pytafast exactly matches TA-Lib (Wilder), differs from TTR scale"
)
@pytest.mark.parametrize("reference_data", DATA_FILES, indirect=True)
...
@pytest.mark.xfail(
    reason="Chaikin Volatility uses wildly different smoothing bases between TA-Lib and TTR"
)
@pytest.mark.parametrize("reference_data", DATA_FILES, indirect=True)
...
@pytest.mark.xfail(
    reason="TA-Lib Parabolic SAR defaults to slightly different internal acceleration padding"
)
@pytest.mark.parametrize("reference_data", DATA_FILES, indirect=True)
...
@pytest.mark.xfail(
    reason="TTR MACD EMA smoothing seeds and initialization scale mismatch TA-Lib"
)
@pytest.mark.parametrize("reference_data", DATA_FILES, indirect=True)
...
@pytest.mark.xfail(
    reason="Williams percent R scale normalization in TTR completely inverses TA-Lib bounds"
)
@pytest.mark.parametrize("reference_data", DATA_FILES, indirect=True)
