...
pytestmark = pytest.mark.skipif(
    _should_skip(), reason="Wolfram Engine not available or running in CI"
)
...
def sample_data(wl_session):
    ...
    df = pd.DataFrame(
        raw_list, columns=["Date", "Open", "High", "Low", "Close", "Volume"]
    )
...
def compare_results(
    py_res, wl_res, rtol=1e-4, atol=1e-8, name="Indicator", skip_head=0
):
    if wl_res is None:
...
def test_dema_alignment(wl_session, sample_data):
    ...
    wl_res = _get_wl_indicator(
        wl_session, sample_data, "DoubleExponentialMovingAverage", period, "Close"
    )
...
def test_tema_alignment(wl_session, sample_data):
    ...
    wl_res = _get_wl_indicator(
        wl_session, sample_data, "TripleExponentialMovingAverage", period, "Close"
    )
...
def test_atr_alignment(wl_session, sample_data):
    ...
    wl_res = _get_wl_indicator(
        wl_session, sample_data, "AverageTrueRange", period, "OHLCV"
    )
...
def test_obv_alignment(wl_session, sample_data):
    ...
    wl_res = _get_wl_indicator(
        wl_session, sample_data, "OnBalanceVolume", period=None, inputs="OHLCV"
    )
...
def test_willr_alignment(wl_session, sample_data):
    ...
    wl_res = _get_wl_indicator(
        wl_session, sample_data, "WilliamsPercentR", period, "OHLCV"
    )
...
def test_mfi_alignment(wl_session, sample_data):
    ...
    wl_res = _get_wl_indicator(
        wl_session, sample_data, "MoneyFlowIndex", period, "OHLCV"
    )
...
def test_adx_alignment(wl_session, sample_data):
    ...
    wl_res = _get_wl_indicator(
        wl_session, sample_data, "AverageDirectionalMovementIndex", period, "OHLCV"
    )
...
def test_ema_alignment(wl_session, sample_data):
    ...
    wl_res = _get_wl_indicator(
        wl_session, sample_data, "ExponentialMovingAverage", period, "Close"
    )
...
def test_rsi_alignment(wl_session, sample_data):
    ...
    wl_res = _get_wl_indicator(
        wl_session, sample_data, "RelativeStrengthIndex", period, "Close"
    )
